...
int main(void)
{
   ...
   // mbedtls_aes_context okokoko;
    ...
    // 16-byte Initialization Vector (IV) for AES-CBC
       uint8_t IV_Buffer[] = {
            0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
            0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88
        };
       uint8_t IV_Copy[16];     // Copy of IV for decryption

           // Copy IV before encryption (because it gets modified during encryption)
  ...
  //  mbedtls_aes_setkey_en( &okokok, Key_Buffer,128);
    mbedtls_aes_crypt_cbc(&aes_context,MBEDTLS_AES_ENCRYPT,16,IV_Buffer,Inp_Buffer,Out_Buffer);
//mbedtls_aes_crypt_ecb(&okokoko,MBEDTLS_AES_ENCRYPT ,Inp_Buffer, unsigned char output[16])
    ...
    mbedtls_aes_crypt_cbc(&aes_context,MBEDTLS_AES_DECRYPT,16,IV_Copy,Out_Buffer,Recv_Buffer);
