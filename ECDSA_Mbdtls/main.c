...
int main(void)
{
    ...
    result = mbedtls_ecp_group_load(&keypair.private_grp, MBEDTLS_ECP_DP_SECP256R1); // not needed (required if the things are on different interface)
    printf("Key pair generation result: %d\r\n", result);
    result = mbedtls_ecp_gen_keypair(&keypair.private_grp, &keypair.private_d, &keypair.private_Q, mbedtls_ctr_drbg_random, &random_context);
    printf("Key generation result: %d\r\n", result);

    /* Print keypair */
    printf("Private key: ");
    print_mpi_data(&keypair.private_d);
    printf("Public key: ");
    print_ecp_point_data(&keypair.private_Q, &keypair.private_grp);
    ...
    /* Print hash */
    printf("Message hash: ");
    print_uint8_data(hash, sizeof(hash));
