...
{
    cy_rslt_t result;
    uint8_t rand_num1[10]={0};
    uint8_t rand_num2[10]={0};
	...
	printf("PSOC_OPTIGA_IOT_KIT template is ready to start.\r\n");
		...
		mbedtls_ctr_drbg_init(&random_context);  // for first array
		mbedtls_entropy_init(&entropy_context);

		mbedtls_ctr_drbg_context random_context1;
		mbedtls_entropy_context entropy_context1;  // for second array
	    mbedtls_ctr_drbg_init(&random_context1);
	    mbedtls_entropy_init(&entropy_context1);


		mbedtls_ctr_drbg_seed(&random_context,mbedtls_entropy_func,&entropy_context,"hello",5);
		mbedtls_ctr_drbg_seed(&random_context1,mbedtls_entropy_func,&entropy_context1,"what",4);
		int flag = mbedtls_ctr_drbg_random(&random_context,rand_num1,sizeof(rand_num1));
        int ret = mbedtls_ctr_drbg_random(&random_context1,rand_num2,sizeof(rand_num2));
		...
		for(int i=0;i<10;i++)
			{
				...
				array3[i]=rand_num1[i]+rand_num2[i];
				 printf("Array1[%d]: %d, Array2[%d]: %d, Sum[%d]: %d",i, rand_num1[i], i, rand_num2[i], i, array3[i]);

			}
			mbedtls_ctr_drbg_free(&random_context);
			mbedtls_entropy_free(&entropy_context);

			mbedtls_ctr_drbg_free(&random_context1);
			mbedtls_entropy_free(&entropy_context1);
